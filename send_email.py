...
def email_tool(receiver, URL, upload_name, status_num, name, user_update):
    ...
    text_detials = \
            ...
            'To get more details, you can visit the following website: https://github.com/plana-planetarium/kemono_sync/actions'
    ...
    def get_text(status, next_status):
        text = \
                '[plana-planetarium/kemono_sync] 自动同步kemono上的画师文件 (%s/3) %s successfully\n\n' % (str(status_num), str(status)) + \
    ...
    def get_subject(status):
        subject = '[plana-planetarium/kemono_sync] (%s/3) %s successfully' % (str(status_num), str(status))
        return subject
